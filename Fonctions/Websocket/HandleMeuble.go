...
func HandleWebsocketMeuble(w http.ResponseWriter, r *http.Request) {
	...
	if username, ok := session.Values["username"].(string); ok {
		...
		oldLikes, err = getOldLikesDislikesFromDBMeuble()
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
...
func saveMessageToDBMeuble(msg Message) error {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		return err
	}
...
func getOldLikesDislikesFromDBMeuble() ([]LikesDislikes, error) {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		return nil, err
	}
	...
	rows, err := db.Query("SELECT id, likes, dislikes FROM meuble ORDER BY id")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func getOldMeubleMessagesFromDB() ([]Message, error) {
	...
	rows, err := db.Query("SELECT id, username, content FROM meuble ORDER BY id")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func incrementLikesMeuble(messageID int) error {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		return err
	}
	...
	_, err = db.Exec("UPDATE meuble SET likes = likes + 1 WHERE id = ?", messageID)
	if err != nil {
		return err
	}
...
func incrementDislikesMeuble(messageID int) error {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		return err
	}
	...
	_, err = db.Exec("UPDATE meuble SET dislikes = dislikes + 1 WHERE id = ?", messageID)
	if err != nil {
		return err
	}
...
func LikeHandlerMeuble(w http.ResponseWriter, r *http.Request) {
	...
	err = incrementLikesMeuble(messageIDInt)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
