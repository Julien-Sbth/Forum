...
func HandleWebsocketEchec(w http.ResponseWriter, r *http.Request) {
	...
	if username, ok := session.Values["username"].(string); ok {
		...
		imageList, err := getAllImageURLsFromDBEchec()
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
...
func getAllImageURLsFromDBEchec() ([]string, error) {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		return nil, err
	}
...
func saveImageToDBEchec(imageData string) error {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		return err
	}
	...
	_, err = db.Exec("INSERT INTO EchecImages (data) VALUES (?)", imageData)
	if err != nil {
		return err
	}
...
func WebSocketHandlerEchec(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if msg.Image != "" {
			err = saveImageToDBEchec(msg.Image)
			if err != nil {
...
func UploadEchec(w http.ResponseWriter, r *http.Request) {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	_, err = db.Exec("INSERT INTO EchecImages (Data) VALUES (?)", imageBase64)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func ImageHandlerEchec(w http.ResponseWriter, r *http.Request) {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
