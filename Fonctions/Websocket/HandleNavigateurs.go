...
func HandleWebsocketNavigateurs(w http.ResponseWriter, r *http.Request) {
	...
	if username, ok := session.Values["username"].(string); ok {
		...
		imageList, err := getAllImageURLsFromDBNavigateurs()
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
...
func getAllImageURLsFromDBNavigateurs() ([]string, error) {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		return nil, err
	}
	...
	rows, err := db.Query("SELECT data FROM NavigateursImages")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func saveImageToDBNavigateurs(imageData string) error {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		return err
	}
	...
	_, err = db.Exec("INSERT INTO NavigateursImages (data) VALUES (?)", imageData)
	if err != nil {
		return err
	}
...
func WebSocketHandlerNavigateurs(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if msg.Image != "" {
			err = saveImageToDBNavigateurs(msg.Image)
			if err != nil {
...
func UploadNavigateurs(w http.ResponseWriter, r *http.Request) {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	...
	_, err = db.Exec("INSERT INTO NavigateursImages (Data) VALUES (?)", imageBase64)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func ImageHandlerNavigateurs(w http.ResponseWriter, r *http.Request) {
	db, err := sql.Open("sqlite3", "database.sqlite")
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
