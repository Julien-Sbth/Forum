...
const socket = new WebSocket("wss://localhost:443/wsNavigateurs");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    likeForm.action = "/likesNavigateurs";
    ...
    dislikeForm.action = "/dislikesNavigateurs";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesNavigateurs');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadNavigateurs', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageNavigateurs?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
