...
const socket = new WebSocket("wss://localhost:443/wsPC");
const ID = "chatBox";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesPC');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadPC', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImagePC?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
