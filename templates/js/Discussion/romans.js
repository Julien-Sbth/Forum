...
const socket = new WebSocket("wss://localhost:443/wsRomans");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesRomans";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesRomans');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadRomans', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageRomans?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
