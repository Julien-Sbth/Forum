...
const socket = new WebSocket("wss://localhost:443/wsRugby");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesRugby";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesRugby');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadRugby', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageRugby?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
