...
const socket = new WebSocket("wss://localhost:443/wsCyber");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesCyber";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesCyber');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadCyber', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageCyber?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
