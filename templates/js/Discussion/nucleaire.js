...
const socket = new WebSocket("wss://localhost:443/wsNucleaire");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    likeForm.action = "/likesNucleaire";
    ...
    dislikeForm.action = "/dislikesNucleaire";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesNucleaire');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadNucleaire', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageNucleaire?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
