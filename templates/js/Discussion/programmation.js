...
const socket = new WebSocket("wss://localhost:443/wsProgrammation");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    likeForm.action = "/likesProgrammation";
    ...
    dislikeForm.action = "/dislikesProgrammation";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesProgrammation');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadProgrammation', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageProgrammation?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
