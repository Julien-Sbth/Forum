...
const socket = new WebSocket("wss://localhost:443/wsEmploie");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesEmploie";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesEmploie');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadEmploie', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageEmploie?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
