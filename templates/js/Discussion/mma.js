...
const socket = new WebSocket("wss://localhost:443/wsMMA");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesMMA";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesMMA');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadMMA', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageMMA?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
