...
const socket = new WebSocket("wss://localhost:443/wsVirus");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesVirus";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesVirus');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadVirus', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageVirus?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
        ...
        if (images && Array.isArray(images) && images.length > 0) {
            ...
            noImageMessage.textContent = "Aucune image disponible pour cet page.";
