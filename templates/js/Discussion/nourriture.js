...
const socket = new WebSocket("wss://localhost:443/wsNourriture");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    likeForm.action = "/likesNourriture";
    ...
    dislikeForm.action = "/dislikesNourriture";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesNourriture');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadNourriture', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageNourriture?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
