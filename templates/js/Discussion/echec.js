...
const socket = new WebSocket("wss://localhost:443/wsEchec");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesEchec";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesEchec');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadEchec', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageEchec?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
