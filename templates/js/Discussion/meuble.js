...
const socket = new WebSocket("wss://localhost:443/wsMeuble");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesMeuble";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesMeuble');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadMeuble', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageMeuble?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
