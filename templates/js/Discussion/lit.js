...
const socket = new WebSocket("wss://localhost:443/wsLit");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesLit";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesLit');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadLit', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageLit?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
