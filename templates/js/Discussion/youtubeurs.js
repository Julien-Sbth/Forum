...
const socket = new WebSocket("wss://localhost:443/wsYoutubeurs");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    likeForm.action = "/likesYoutubeurs";
    ...
    dislikeForm.action = "/dislikesYoutubeurs";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesYoutubeurs');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadYoutubeurs', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageYoutubeurs?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
