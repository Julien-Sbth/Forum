...
const socket = new WebSocket("wss://localhost:443/wsMirroir");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesMirroir";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesMirroir');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadMirroir', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageMirroir?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
