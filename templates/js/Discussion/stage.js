...
const socket = new WebSocket("wss://localhost:443/wsStage");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesStage";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesStage');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadStage', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageStage?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
