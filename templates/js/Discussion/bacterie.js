...
const socket = new WebSocket("wss://localhost:443/wsBacterie");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesBacterie";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesBacterie');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadBacterie', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageBacterie?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
