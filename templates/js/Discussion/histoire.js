...
const socket = new WebSocket("wss://localhost:443/wsHistoire");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesHistoire";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesHistoire');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadHistoire', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageHistoire?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
