...
const socket = new WebSocket("wss://localhost:443/wsBox");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesBox";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesBox');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadBox', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageBox?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
