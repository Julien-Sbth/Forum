...
const socket = new WebSocket("wss://localhost:443/wsFoot");
const ID = "chatBox";
...
function displayMessage(msg) {
    ...
    dislikeForm.action = "/dislikesFoot";
...
async function fetchLikesDislikes() {
    try {
        const response = await fetch('/LikesDislikesFoot');
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
...
function envoyerImage() {
    ...
    fetch('https://localhost:443/uploadFoot', {
        method: 'POST',
        body: formData
    })
        .then(response => {
...
async function getImagesForId(id) {
    try {
        const response = await fetch(`/getImageFoot?id=${id}`);
        if (!response.ok) {
            throw new Error('Network response was not ok.');
        }
